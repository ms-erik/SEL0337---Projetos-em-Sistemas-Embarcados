...
#include "distance.h"
#include "../common/TRUTH.h"            // Arquivo compartilhado
static const sensor_type_t DEVICE_TYPE = SENSOR_DISTANCE;
static const int DEVICE_ID = 0001;
...
void distance_task(void *pvParameters) {
    ...
    while (1) {
        // bool presence_detected = detect_presence();
        // ESP_LOGI(sensor_type_to_string(DEVICE_TYPE), "Presença detectada: %s", presence_detected ? "Sim" : "Não");
        
        ESP_LOGI(sensor_type_to_string(DEVICE_TYPE), "Presença detectada: NÃO, %d", DEVICE_ID);

        // if (presence_detected) {
        //     ESP_LOGI(sensor_type_to_string(DEVICE_TYPE), "Publicando presença via BLE...");
        //     ble_publish(true, DEVICE_ID, DEVICE_TYPE);
        // }
